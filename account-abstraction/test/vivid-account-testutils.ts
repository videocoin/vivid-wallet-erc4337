...
import {
  ...
  VividAccount__factory, VividAccountFactory, TestAggregatedAccountFactory
} from '../typechain'
...
// create non-random account, so gas calculations are deterministic
export function createAccountAdmin (): Wallet {
  const privateKey = keccak256(Buffer.from(arrayify(BigNumber.from(++counter))))
  return new ethers.Wallet(privateKey, ethers.provider)
  // return new ethers.Wallet('0x'.padEnd(66, privkeyBase), ethers.provider);
}
...
export function getAccountInitCode (owner: string, userAddress: string, emailHash: BigNumber, factory: VividAccountFactory, salt = 0): BytesLike {
  return hexConcat([
    factory.address,
    factory.interface.encodeFunctionData('createAccount', [emailHash, salt])
  ])
}
...
export async function getAccountAddress (emailHash: string, factory: VividAccountFactory, salt = 0): Promise<string> {
  return await factory.getAddress(emailHash,  salt)
}
...
export async function createAccount (
  ...
  emailHash: string,
  entryPoint: string,
  _factory?: VividAccountFactory
):
  Promise<{
    proxy: VividAccount
    accountFactory: VividAccountFactory
    implementation: string
  }> {
  const accountFactory = _factory ?? await new VividAccountFactory__factory(ethersSigner).deploy(admin, entryPoint)
  const implementation = await accountFactory.accountImplementation()
  await accountFactory.createAccount(emailHash, 0)
  const accountAddress = await accountFactory.getAddress(emailHash, 0)
  const proxy = VividAccount__factory.connect(accountAddress, ethersSigner)
  return {
    implementation,
    accountFactory,
    proxy

...
The ERC4377 specification[1] describes the operation of the above components. A brief overview is here:
* Bundlers: A bundler collects UserOperations from users and submit them to an EntryPoint contract. Generally it is part of P2P network that operates on the concept alt-memory pool explained in the specification.
* EntryPoint: A contract that receives UserOperations from bundlers, validates them, pays for gas, and executes them. It is a secure and audited component. It calls the user's wallets to perform the validated UserOperation
...
* Wallets: smart contract accounts that can execute transactions with UserOperations. They have logic to verify signatures, update nonces, and perform actions. The Wallet can include logic beyond traditional ECDSA signature verification to support web2 authentication schemes.
* Paymasters: Contracts that pay for the gas fees of UserOperations on behalf of wallets. They can implement various payment schemes such as tokens, subscriptions, etc. These contracts work based on signatures generated by offline Paymaster components and embedded in the UserOperation. 

 We leverage the open source implementation of some of these components and modified as described below.
